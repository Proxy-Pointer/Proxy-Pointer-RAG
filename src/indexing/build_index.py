...
Combined pipeline that:
  Step 0: Builds skeleton trees for new .md files (via PageIndex)
...
from src.config import (
    DATA_DIR, TREES_DIR, INDEX_DIR, PAGEINDEX_DIR,
...
def build_skeleton_trees(data_dir, trees_dir):
    """
    ...
    for file in md_files:
        ...
        cmd = [
            sys.executable,
            os.path.join(str(PAGEINDEX_DIR), "run_pageindex.py"),
            "--md_path", md_path,
        ...
        process = subprocess.run(
            cmd, capture_output=True, text=True, cwd=str(PAGEINDEX_DIR)
        )

        if process.returncode == 0:
            source_json = os.path.join(
                str(PAGEINDEX_DIR), "results", f"{base_name}_structure.json"
            )
            if os.path.exists(source_json):
                ...
                logging.error(f"  -> Expected JSON at {source_json} but not found.")
        else:
            logging.error(f"  -> Failed building tree for {file}.")
